...
export default async function trading({
  ...
  if (
    ...
    const percent = 0.5; //내 보유현금의 %만 쓰겠다.
    const finalPercent = percent === 1 ? 0.9975 : percent;
    const availableBuyVolume = (intMyCash * finalPercent) / askPrice;
    const fourDecimalVolume = String(
      Math.floor(availableBuyVolume * 10000) / 10000
    );
    console.log(askPrice, intMyCash, finalPercent, fourDecimalVolume);
    ...
    if (marketBuyId) {
      const buyDetail = await getOrderDetail(marketBuyId, coinName);
      //주문했던 수량
      ...
      // const sellDetail = await getOrderDetail(marketBuyId, coinName);
      return limitSellId;
    }
  }
  return;
